...
Authentication is required to access protected endpoints in the `{settings.title}` API.

## Getting credentials

To use the API, you need a public key and a private key. These credentials can be generated in your account dashboard.

1. Access the `{settings.title}` dashboard.
...
If you have questions about credentials or authentication, contact us at `{settings.supportEmail}`.
